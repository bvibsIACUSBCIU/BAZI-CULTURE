...
export class AuthService {
  ...
  generateChallenge(walletAddress, { operation, username, origin } = {}) {
    ...
    if (!['register', 'login', 'authenticate'].includes(cleanOperation)) {
      const err = new Error('INVALID_AUTH_OPERATION');
      err.code = 'INVALID_AUTH_OPERATION';
      throw err;
    }
    const cleanUsername = cleanOperation === 'authenticate' ? null : this.normalizeUsername(username);
    const cleanOrigin = this.normalizeOrigin(origin);
    const cleanAddress = walletAddress.toLowerCase();
    const challengeId = crypto.randomUUID();
    const timestamp = Date.now();
    const usernameLine = cleanUsername ? `\nUsername: ${cleanUsername}` : '';
    const challenge = `Tianfu-Bazi Auth Challenge\nOperation: ${cleanOperation.toUpperCase()}${usernameLine}\nWallet: ${cleanAddress}\nOrigin: ${cleanOrigin}\nNonce: ${challengeId}\nTimestamp: ${timestamp}`;
  ...
  verifySignature(challengeId, walletAddress, signature, { operation, username, origin } = {}) {
    ...
    try {
      ...
      if (record.operation !== 'authenticate' && record.username !== this.normalizeUsername(username)) return false;
  ...
  authenticate(walletAddress, clientIp = '127.0.0.1') {
    const cleanAddress = walletAddress.toLowerCase();
    const existing = this.accounts.get(cleanAddress);
    ...
    registeredWallets.add(cleanAddress);
    this.ipToWallets.set(cleanIp, registeredWallets);
    this.accounts.set(cleanAddress, account);
    return account;
  }
