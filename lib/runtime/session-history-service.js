...
export class SessionHistoryService {
  ...
  updateSession(walletAddress = 'default_user', sessionId, patch = {}) {
    const session = this.getSessions(walletAddress).find((item) => item.id === sessionId);
    if (!session) return null;
    for (const key of ['title', 'question', 'topic', 'summary', 'reportMarkdown']) {
      if (typeof patch[key] === 'string') session[key] = patch[key];
    }
    return session;
  }
}
