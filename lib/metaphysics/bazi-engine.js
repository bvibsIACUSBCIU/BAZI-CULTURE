...
export const CALCULATION_POLICY = Object.freeze({
  ...
  timeBoundary: "23:00-00:59 is calculated with the supplied civil date in Asia/Shanghai (UTC+8)",
...
export function calculateChartRelations(pillars) {
  ...
  const seenSelfPunishments = new Set();
  ...
  for (let leftIndex = 0; leftIndex < entries.length; leftIndex += 1) {
    for (
      ...
      if (
        ...
        if (!seenSelfPunishments.has(left.branch)) {
          seenSelfPunishments.add(left.branch);
          branchRelations.push(
            Object.freeze({
              type: "自刑",
              positions: [left.position, right.position],
  ...
  const branchGroups = BRANCH_GROUP_RELATIONS.filter((group) =>
    ...
    Object.freeze({
      ...group,
      positions: [...group.branches]
        .map((branch) => entries.find((entry) => entry.branch === branch)?.position)
        .filter(Boolean),
...
export function countSurfaceElements(pillars) {
  ...
  for (const pillar of [pillars?.year, pillars?.month, pillars?.day, pillars?.time]) {
    if (typeof pillar !== "string" || !PILLAR_PATTERN.test(pillar)) continue;
