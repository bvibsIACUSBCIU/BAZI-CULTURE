export function createRepositories(db, { now = () => new Date().toISOString(), createId = defaultCreateId } = {}) {
  ...
  const reports = createReportRepository(db, { now, createId });
  const checkins = createCheckinRepository(db, { now });

  return { users, profiles, conversations, preferences, credits, sessions, reports, checkins };
}
...
function createReportRepository(db, { now, createId }) {
  return {
