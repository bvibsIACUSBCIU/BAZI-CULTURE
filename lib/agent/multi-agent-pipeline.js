...
export async function run6StagePipeline({
  ...
  for (const topicItem of taskPlan.topics || []) {
    ...
    for (const group of topicItem.groups || []) {
      ...
      const resolvedChartData = extractChartDataForScope(evidencePayload, group.data_scope);
...
function extractChartDataForScope(evidencePayload) {
...
function collectChartFacts(system, payload) {
  ...
  for (const [field, value] of Object.entries(payload)) {
    if (field === "calculationPolicy" || value === undefined || value === null) continue;
    facts.push(createCalculatedFact(`${system}.${field}`, system, field, value));
  }
  return facts;
}
