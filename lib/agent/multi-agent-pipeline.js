...
export async function run6StagePipeline({
  ...
  const minimumStageDelayMs = Number.isFinite(stageDelayMs) ? Math.max(0, stageDelayMs) : 5000;
  const startPhase = (stage) => {
    emit("phase_start", { stage });
    return Date.now();
  };
  const finishPhase = async (stage, startedAt) => {
    const remainingMs = minimumStageDelayMs - (Date.now() - startedAt);
    if (remainingMs > 0) await sleep(remainingMs);
    emit("phase_done", { stage });
  };

  // 100% 确定性排盘计算（存库/代码，不经过 AI）
  const chartPhaseStartedAt = startPhase(0);
  ...
  await finishPhase(1, planningPhaseStartedAt);

  const processedTopics = [];

  // ③【Stage 3: 组分析】LLM 调用（每个 group 一次）
  const analysisPhaseStartedAt = startPhase(2);
  ...
  await finishPhase(2, analysisPhaseStartedAt);

  // ④【Stage 4: 报告撰写/修订】LLM 调用 1 次
  const reportPhaseStartedAt = startPhase(3);
  ...
  const visibleServiceDegradation = Boolean(serviceDegradation && serviceDegradation.presentation !== "full_report");
  if (serviceDegradation && visibleServiceDegradation) {
  ...
  emit("report_done", {
    ...
    serviceDegraded: visibleServiceDegradation,
  });
  await finishPhase(3, reportPhaseStartedAt);

  // ⑤【Stage 5: 对话区总结】直接从已经生成并校验过的报告提取，避免额外网络失败。
  const summaryPhaseStartedAt = startPhase(4);
  ...
  await finishPhase(4, summaryPhaseStartedAt);

  // ⑥【Stage 6: 追问推荐】本地生成安全候选问题。
  const recommendPhaseStartedAt = startPhase(5);
  ...
  await finishPhase(5, recommendPhaseStartedAt);
  ...
  return {
    ...
    service: {
      degraded: visibleServiceDegradation,
