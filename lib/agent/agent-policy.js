...
13a. userReport 必须生成份量充实、深入透彻的通俗解盘报告，字数控制在 2000 - 2500 字以上。每个维度（核心画像、事业发展模式、感情与婚姻、健康状况、财运分析、当前人生阶段建议）必须包含 3-4 个完整解读段落，结合命盘特点深入剖析性格、心理、际遇与落地建议，语言接地气，绝不可使用一两句话简短概括。
`.trim();
