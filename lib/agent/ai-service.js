...
const AI_TIMEOUT_MS = () => clampTimeout(envValue("AI_TIMEOUT_MS", "20000"));
...
export async function callReportWriter({
  ...
  const fallback = (reason, error = null) => {
    ...
    return buildConciseReportFallback({
      year,
      profile,
      question,
      evidencePayload,
...
function validateMarkdownReportAgainstEvidence(markdown, evidencePayload) {
  ...
  if (evidencePayload?.annual?.available !== true && containsUnsupportedTimeBoundEventClaim(text)) {
...
function buildConciseReportFallback({ year, profile, question, evidencePayload, reason }) {
  const factIndex = buildCalculatedFactIndex(evidencePayload);
  ...
  const confirmed = [
    ...
    dayMaster?.stem ? `日主：${dayMaster.stem}${dayMaster.element || ""}` : "",
    elements ? `五行表层计数：${Object.entries(elements).map(([key, value]) => `${key}${value}`).join("、")}` : "",
  ].filter(Boolean);
  const titleYear = Number.isInteger(year) ? `${year}年 ` : "";
  return `# ${titleYear}${name} · 简要盘面说明

> ${reasonText}，以下只展示本次程序已经计算出的盘面摘要；专业综合解读暂未生成，不用重复模板冒充完整报告。
...
针对“${questionText}”，这些原局数据只能提供分析起点，不能单独确认具体行业、收入、关系结果或某一年会发生的事件。建议暂时保留问题，待 AI 服务恢复后再结合八字、紫微与奇门的完整计算结果生成专业解释。

## 当前边界

本轮没有计算流年、大运或年度事件；若问题涉及职业、投资、健康或关系决定，请以现实资料和专业意见为主。`;
}
...
function clampTimeout(value) {
  const parsed = Number.parseInt(String(value || "20000"), 10);
