...
针对“${questionText}”，本轮可以${topicLead}。已排出的四柱为${pillars.join("、") || "未完整取得"}${dayMaster?.stem ? `，日主为${dayMaster.stem}${dayMaster.element || ""}` : ""}。从命理角度看，重点不在给出单一的吉凶判词，而在于借由盘中已经出现的结构，找到更适合当下的节奏、取舍和相处方式。

## 命盘基础

日主${dayMaster?.stem || "未取得"}${dayMaster?.element || ""}是本盘的观察中心，日柱${dayPillar || "未取得"}则提示亲密关系、日常选择与自我感受要放在同一处看。五行表层计数为${elements ? Object.entries(elements).map(([key, value]) => `${key}${value}`).join("、") : "未完整取得"}；它反映的是本次四柱表层的分布，不等同于“缺什么就补什么”。${relations && relations !== "未记录干支关系" ? `盘中还记录到${relations}，这说明某些关系与节奏容易同时被触动，处理问题时更需要分清轻重。` : "本轮未取得可单列的干支关系。"}
...
${ziweiDetails.length ? `紫微部分可见${ziweiDetails.join("；")}。这些位置适合用来观察本题中的角色、互动环境与心理期待，但不宜单独断定结果。` : "紫微部分在本题中没有可稳定引用的宫位摘要，因此以八字事实为主。"}${qimenDetails ? `奇门起局为${qimenDetails}，更适合提示当下处理事情的方式：先看信息是否完整，再决定推进、等待或沟通的节奏。` : "奇门部分没有额外结构可供展开。"}三盘放在一起的意义，是帮助你把感受、选择和现实条件看得更立体，而不是替你做决定。
