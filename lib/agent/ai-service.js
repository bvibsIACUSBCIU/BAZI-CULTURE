...
function buildCompleteReportFallback({ year, profile, question, evidencePayload, reason }) {
  ...
  const dayPillar = fact("bazi.pillars.day");
  const relations = formatEvidenceValue(fact("bazi.relations"));
  const name = profile?.name || "命主";
  ...
  const topicReading = buildFinalTopicReading({
    topic,
  ...
  const practiceFocus = {
    "健康专题": "养生贵在先减耗、后滋养。先把睡眠、饮食和运动做成能长期坚持的基本盘，再观察哪些安排真正让精神变稳。不要把偶发不适都归到命盘上，也不要因为忙碌而忽略身体连续发出的信号。",
    "姻缘专题": "感情的关键不在猜测对方，而在把需要、底线和承诺放到明处。能被说清的误会，往往不必靠忍耐解决；能共同承担的现实事务，才会让关系走得更稳。",
    "事业与行业专题": "职业选择宜看长线，不只看眼前的机会。能让你持续积累、得到清晰反馈、并保留生活余地的环境，通常比短期光鲜却持续透支的选择更适合。",
    "财富专题": "理财先守住本金和现金流，再谈放大收益。任何让你睡不着的杠杆、借贷或合作，都应先把最坏情况写清楚；心定，判断才不会失准。",
    "核心诉求专题": "命盘提供的是观察角度，真正改变处境的仍是清楚的目标、稳定的行动和持续的复盘。把能掌控的事先做好，答案会比反复求一个确定结论更快显现。",
  }[topic];
  const masterNote = buildMasterNote({ topic, dayMaster, dayPillar, elements });
  const baziEvidenceMarker = buildFallbackEvidenceMarker([
    pillars.length && "八字·四柱",
    dayMaster?.stem && "八字·日主",
    elements && "八字·五行分布",
    relations && relations !== "未记录干支关系" && "八字·干支关系",
  ]);
  const crossEvidenceMarker = buildFallbackEvidenceMarker([
    pillars.length && "八字·四柱",
    ziweiDetails.length && "紫微·相关宫位",
    qimenDetails && "奇门·本次起局",
  ]);
...
针对“${questionText}”，本轮可以${topicLead}。已排出的四柱为${pillars.join("、") || "未完整取得"}${dayMaster?.stem ? `，日主为${dayMaster.stem}${dayMaster.element || ""}` : ""}。从命理角度看，重点不在给出单一的吉凶判词，而在于借由盘中已经出现的结构，找到更适合当下的节奏、取舍和相处方式。${baziEvidenceMarker}

## 命盘基础

日主${dayMaster?.stem || "未取得"}${dayMaster?.element || ""}是本盘的观察中心，日柱${dayPillar || "未取得"}则提示亲密关系、日常选择与自我感受要放在同一处看。五行表层计数为${elements ? Object.entries(elements).map(([key, value]) => `${key}${value}`).join("、") : "未完整取得"}；它反映的是本次四柱表层的分布，不等同于“缺什么就补什么”。${relations && relations !== "未记录干支关系" ? `盘中还记录到${relations}，这说明某些关系与节奏容易同时被触动，处理问题时更需要分清轻重。` : "本轮未取得可单列的干支关系。"}${baziEvidenceMarker}
...
${topicReading.analysis}

## 三盘交叉观察

${ziweiDetails.length ? `紫微部分可见${ziweiDetails.join("；")}。这些位置适合用来观察本题中的角色、互动环境与心理期待，但不宜单独断定结果。` : "紫微部分在本题中没有可稳定引用的宫位摘要，因此以八字事实为主。"}${qimenDetails ? `奇门起局为${qimenDetails}，更适合提示当下处理事情的方式：先看信息是否完整，再决定推进、等待或沟通的节奏。` : "奇门部分没有额外结构可供展开。"}三盘放在一起的意义，是帮助你把感受、选择和现实条件看得更立体，而不是替你做决定。${crossEvidenceMarker}
...
${topicReading.actions}

## 落地之法

不要急于把这份报告变成结论，先把建议落实为一个月内能观察的行动。每周选一项重点记录：健康看睡眠和精力，感情看沟通与承诺，事业看投入和反馈，财富看现金流与风险。一个月后回看，哪些做法让状态更稳，哪些反而增加消耗，再据此调整。命理的用处，是帮你看见自己的节奏；真正把日子过顺，还要靠清醒、耐心和持续行动。

同时要记得，盘中任何一个符号都不是孤立存在的。日主、日柱、五行与三盘结构只能共同构成观察角度，不能取代现实中的健康检查、沟通回应、职业能力或财务数据。把命理当作一盏提醒方向的灯，而不是替你下判决的尺子，既能保留传统智慧，也能让每一步走得更踏实。

## 收束建议

${topicReading.closing}本轮没有计算流年、大运或具体事件，因此不判断某一年一定发生的婚姻、收入、职业或健康结果。把本报告用于整理重点和安排现实行动，会比把它当作绝对结论更有价值。`;
}

function buildFallbackEvidenceMarker(labels) {
  const available = [...new Set(labels.filter(Boolean))];
  return available.length ? `〔依据：${available.join("；")}〕` : "";
}

function buildFinalTopicReading({ topic, question, dayMaster, dayPillar, elements }) {
  const elementText = elements ? Object.entries(elements).map(([key, value]) => `${key}${value}`).join("、") : "当前计数未完整取得";
  const master = `${dayMaster?.stem || "日主"}${dayMaster?.element || ""}`;
  const day = dayPillar || "日柱";
  const readings = {
    "健康专题": {
      analysis: `关于“${question}”，以${master}为中心、配合${elementText}来看，报告更建议你把精力问题理解为“消耗与恢复是否平衡”，而不是简单按五行多少下结论。盘中火的表层数量较多时，日常容易出现事情一多就节奏偏快、休息被压缩的情况；水与金的记录则提醒，恢复质量、睡眠连续性和呼吸节奏同样重要。${day}落在日常感受的位置，说明身体的反馈往往会先于意志提醒你：心烦、入睡拖延、食欲波动或专注力下降，都值得及早调整。`,
      actions: `1. 固定起床时间，先保证连续七天的睡眠窗口，再谈补救性熬夜。\n2. 将高强度工作集中在精神较好的两段时间，中间安排十到二十分钟离屏、走动或安静呼吸。\n3. 饮食上以规律、温和和可持续为先，不建议按五行标签自行极端进补或节食。\n4. 连续两周记录睡眠、心率、胃口、运动量和情绪波动；若疲劳、疼痛或失眠持续，应优先咨询医生。`,
      closing: `这份盘面给出的重点，是让你把“硬撑”改为“有节律地用力”。精力管理做得好，工作、人际和情绪自然会更稳。`,
    },
    "姻缘专题": {
      analysis: `关于“${question}”，${day}是本题的重要观察点。它更适合帮助你理解自己在亲密关系中重视的安全感、回应方式与相处节奏，而不是替你判断感情一定好或一定坏。以${master}与${elementText}为背景，关系里既可能有主动表达的一面，也需要留出让情绪沉淀和彼此确认的空间。若遇到争执，先分辨眼前是事实分歧、期待落差，还是沟通方式过急；把三者混在一起，最容易让小事变成消耗。`,
      actions: `1. 把最在意的三件事说清楚，例如联系频率、金钱边界和遇事的处理方式。\n2. 发生冲突时先暂停情绪化回应，约定一个可继续沟通的时间，不用冷处理代替答案。\n3. 每周留出一次不讨论问题的相处时间，重新积累轻松和信任。\n4. 若关系长期出现贬低、控制或暴力，应优先寻求现实支持，不用命盘为伤害找理由。`,
      closing: `真正有利于感情的，不是追问一个注定的结局，而是把关系经营成双方都能安心表达、也愿意承担责任的状态。`,
    },
    "事业与行业专题": {
      analysis: `关于“${question}”，${master}与${elementText}提供的是工作节奏和环境偏好的观察线索，不是某个行业的录取通知。更值得比较的是：岗位是否有清晰目标、能否积累可迁移的技能、协作规则是否明确，以及你的精力能否支撑长期投入。盘中结构出现多种十神和干支关系时，往往说明同一份工作会同时带来机会与约束；不要只因短期热情就忽略团队、收入、成长路径和生活成本。`,
      actions: `1. 用职责、成长、收入、通勤和恢复成本五项指标比较具体岗位。\n2. 先做小范围试岗、项目合作或作品验证，再决定是否转向。\n3. 把每周最有成就感和最耗竭的工作分别记录，三周后复盘。\n4. 涉及离职或创业时，先准备现金流与替代方案，避免在情绪高点做不可逆决定。`,
      closing: `适合你的选择，应当让能力、节奏和现实资源能持续配合，而不是只靠一时冲劲。`,
    },
    "财富专题": {
      analysis: `关于“${question}”，${master}与${elementText}只能帮助整理你对资源、压力和节奏的看法，不能据此预测收益或推荐买卖。财富问题的核心仍是现金流、负债、收入稳定性和风险承受能力。盘中出现的财星、食伤或比劫名称，可以作为你观察自己在获取资源、合作分配和消费冲动上的提醒；真正的结论要回到合同、预算和账户记录。`,
      actions: `1. 先建立三个月支出表，区分固定支出、弹性支出和风险支出。\n2. 投资或借贷前，写下最大可承受损失和退出条件。\n3. 合作涉及金钱时，提前明确分工、分账与违约处理。\n4. 每月固定复盘一次现金流，不在焦虑或兴奋时追加高风险决策。`,
      closing: `财务稳定来自规则、记录和长期纪律，不来自一条命盘结论。`,
    },
    "核心诉求专题": {
      analysis: `关于“${question}”，${master}、${day}与${elementText}提供了本次可参考的结构起点。它们提示你先看清自己目前处在什么节奏、有哪些现实资源、哪些关系或责任正在占用精力。命理的价值在于帮助你提出更准确的问题，而不是替你跳过现实验证。把问题从“结果会不会发生”转成“我现在能做什么”，往往更有力量。`,
      actions: `1. 写下本题最想得到的结果、最担心的风险和可接受的代价。\n2. 为目标设一个两周内可完成的小行动，再根据反馈调整。\n3. 遇到重大决定时，同时听取专业人士和可信任关系人的意见。\n4. 保留记录，避免只凭某一次情绪或单一解释改变长期计划。`,
      closing: `当命盘提示与现实反馈一起使用时，它更像一面镜子，帮助你把注意力放回真正能改善的地方。`,
    },
  };
  return readings[topic] || readings["核心诉求专题"];
}

function buildMasterNote({ topic, dayMaster, dayPillar, elements }) {
  const master = `${dayMaster?.stem || "日主"}${dayMaster?.element || ""}`;
  const day = dayPillar || "日柱";
  const elementText = elements ? Object.entries(elements).map(([key, value]) => `${key}${value}`).join("、") : "当前计数未完整取得";
  const notes = {
    "健康专题": `从命理师的角度，${master}落在${day}，配合${elementText}，看的是你在繁忙与休息之间如何调度自己。表层火多，不宜一味追求更快、更满；金水并见，也提醒你保留安静、规律和收束的时间。土的数量为零，并不等于必须“补土”，而是提示生活里更需要稳定的饮食、作息和环境感。健康调理的关键是让身体知道何时启动、何时停下：白天有输出，夜里能安定，精力才不会被琐事持续抽空。`,
    "姻缘专题": `从命理师的角度，${master}落在${day}，关系中最重要的不是替你预告桃花数量，而是看你如何安放期待。${elementText}显示的只是表层结构；真正决定感情走向的，是能否把喜欢、顾虑和底线说清。若一方总在猜、另一方总在躲，再好的缘分也会被消耗。你的功课是把情绪从即时反应变成可沟通的语言，既保留温度，也保留分寸。`,
    "事业与行业专题": `从命理师的角度，${master}落在${day}，工作选择要看长期节奏是否合身。${elementText}不是职业标签，却能提醒你观察自己在高压、协作和独立任务中的真实状态。适合的工作不是每天都轻松，而是在辛苦之后仍能积累能力、看见反馈，并保有重新整理的余地。与其急着寻找“天命行业”，不如把可迁移技能、可靠关系和稳定收入一点点做厚。职业上的取舍，尤其要分清“眼前有人需要我”与“这件事值得我长期投入”并不是同一回事：前者容易把人推入忙乱，后者才能沉淀作品、方法和信誉。若新机会只有更高强度、却没有更清晰的职责和成长空间，便应多看一步，不必急于把短期热度当成方向。`,
    "财富专题": `从命理师的角度，${master}落在${day}，财务问题先看心性与节奏，再看机会。${elementText}不能直接推出发财或破财，却能提醒你：一旦压力上来，最容易忽略的是现金流和风险边界。守财并非保守，而是让自己在选择面前不被短期情绪逼迫。账目清楚、承诺写明、风险可控，才是把资源留在手里的根本。`,
    "核心诉求专题": `从命理师的角度，${master}落在${day}，意味着做判断时既要看外部机会，也要听见自己的真实感受。${elementText}是盘面结构的一个切口，不是命运的最终判决。眼下最值得做的，是把想要的结果、能承担的代价和已经拥有的资源放在一起衡量。心里有秩序，行动才不会被一时的焦虑牵着走。`,
  };
  return notes[topic] || notes["核心诉求专题"];
}
